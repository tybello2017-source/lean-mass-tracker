...
function migrate(){
 ...
 state.exerciseLibrary=seed.exerciseLibrary||state.exerciseLibrary||[];
 // V2 keeps the user's current workout programme exactly as stored. Editing a workout must never rewrite historical logs.
...
function saveCheckin(){const l=logFor(selectedDate);l.weight=numOrNull($('#todayWeight').value);l.sleep=sleepFromInputs($('#todaySleepH').value,$('#todaySleepM').value);l.waist=numOrNull($('#todayWaist').value);l.chest=numOrNull($('#todayChest').value);l.arm=numOrNull($('#todayArm').value);l.thigh=numOrNull($('#todayThigh').value);l.energy=$('#todayEnergy').value;save();renderAll();toast('Check-in saved')}
...
function renderWorkouts(){
 ...
 $('#view-workouts').innerHTML=`<div class="section-title"><div><span class="muted">Editable home programme · V2.0</span><h2>Workout ${k}</h2></div><span class="pill">${log.training==='Yes'&&log.workout===k?'Completed':'3 / week'}</span></div>
 ...
 <div class="tabbar">${buttons}</div><div class="workout-editbar"><div><b>Programme editor</b><span>Change exercises, sets, reps and order without changing past sessions.</span></div><button class="secondary compact" onclick="openWorkoutEditor('${k}')">Edit Workout ${k}</button></div>
 ...
 <div class="notice"><b>V2.0:</b> your programme is now editable. Historical sessions stay tied to their saved exercise names, so changing today's programme does not relabel your old training records.</div>`;
}
...
function openWorkoutEditor(k){
 state.editWorkout=k;renderWorkoutEditor();$('#workoutEditorDialog').showModal();
}
function renderWorkoutEditor(){
 const k=state.editWorkout||state.uiWorkout||'A',items=state.workouts[k]||[];
 $('#workoutEditorBody').innerHTML=`<div class="dialog-head"><button class="text-btn" onclick="$('#workoutEditorDialog').close()">Close</button><h3>Edit Workout ${k}</h3><button class="text-btn strong" onclick="$('#workoutEditorDialog').close();renderWorkouts()">Done</button></div><p class="muted">Edit the current programme only. Your completed workout history remains unchanged.</p><div class="editor-list">${items.map((x,i)=>`<div class="editor-row"><div class="editor-title"><b>${i+1}. ${esc(x.exercise)}</b><span>${esc(exerciseCategory(x.exercise))}</span></div><div class="editor-fields"><label>Sets<input id="edit-sets-${i}" type="number" min="1" max="8" value="${x.sets}"></label><label>Reps<input id="edit-reps-${i}" value="${esc(x.reps)}"></label></div><label>Note<input id="edit-note-${i}" value="${esc(x.note||'')}"></label><div class="editor-actions"><button class="ghost compact" ${i===0?'disabled':''} onclick="moveWorkoutExercise('${k}',${i},-1)">↑ Up</button><button class="ghost compact" ${i===items.length-1?'disabled':''} onclick="moveWorkoutExercise('${k}',${i},1)">↓ Down</button><button class="secondary compact" onclick="saveWorkoutExerciseEdit('${k}',${i})">Save</button><button class="ghost compact danger-text" onclick="deleteWorkoutExercise('${k}',${i})">Remove</button></div></div>`).join('')}</div><button class="primary full" onclick="$('#workoutEditorDialog').close();openExerciseLibrary('${k}')">＋ Add exercise from library</button>`;
}
function saveWorkoutExerciseEdit(k,i){const x=state.workouts[k]?.[i];if(!x)return;x.sets=Math.max(1,+$(`#edit-sets-${i}`).value||x.sets);x.reps=$(`#edit-reps-${i}`).value.trim()||x.reps;x.note=$(`#edit-note-${i}`).value.trim();save();renderWorkoutEditor();renderWorkouts();toast('Workout updated')}
function moveWorkoutExercise(k,i,dir){const a=state.workouts[k]||[],j=i+dir;if(j<0||j>=a.length)return;[a[i],a[j]]=[a[j],a[i]];save();renderWorkoutEditor();renderWorkouts()}
function deleteWorkoutExercise(k,i){const x=state.workouts[k]?.[i];if(!x)return;if(confirm(`Remove ${x.exercise} from Workout ${k}? Past logged sessions will be kept.`)){state.workouts[k].splice(i,1);save();renderWorkoutEditor();renderWorkouts();toast('Exercise removed')}}
...
function renderProgress(){
 ...
 $('#view-progress').innerHTML=`<div class="section-title"><div><span class="muted">Connected health & training dashboard · V2.0</span><h2>Progress</h2></div></div>
 ...
 <div class="progress-insight"><div><span>Current weight</span><b>${latest??'—'} kg</b></div><div><span>Change from start</span><b>${latest==null?'—':((latest-start)>=0?'+':'')+(latest-start).toFixed(1)+' kg'}</b></div><div><span>Goal remaining</span><b>${latest==null?'—':Math.max(0,goal-latest).toFixed(1)+' kg'}</b></div></div><div class="card"><h3>Weight trend</h3><div class="canvas-wrap"><canvas id="weightChart" width="760" height="320"></canvas></div><div class="muted">Target path: ${start} → ${goal} kg across 12 weeks.</div></div>
 ...
 <div class="card"><h3>Recovery & consistency</h3><div class="weekly-summary four"><div><span class="muted">Avg sleep</span><b>${(()=>{const sv=week.map(d=>state.logs[d]?.sleep).filter(v=>v!=null);return sv.length?sleepText(sv.reduce((a,b)=>a+b,0)/sv.length):'—'})()}</b></div><div><span class="muted">Training</span><b>${week.filter(d=>state.logs[d]?.training==='Yes').length}/3</b></div><div><span class="muted">Avg kcal</span><b>${n?Math.round(kcal/n):'—'}</b></div><div><span class="muted">Avg protein</span><b>${n?Math.round(prot/n)+'g':'—'}</b></div></div><p class="footer-note">Sleep, nutrition and completed workouts are shown together so you can interpret weight and training changes in context.</p></div><div class="card"><h3>This week</h3><div class="weekly-summary"><div><span class="muted">Avg calories</span><b>${n?Math.round(kcal/n):'—'}</b></div><div><span class="muted">Avg protein</span><b>${n?Math.round(prot/n)+'g':'—'}</b></div><div><span class="muted">Workouts</span><b>${week.filter(d=>state.logs[d]?.training==='Yes').length}/3</b></div></div></div>
 <div class="card"><h3>Meal photos</h3><div id="photoGallery" class="photo-gallery"><div class="empty">Loading photos…</div></div></div>
 <div class="card"><h3>Recent check-ins</h3>${entries.slice(-12).reverse().map(([d,x])=>`<div class="history-row row between"><div><strong>${fmtDate(d)}</strong><div class="muted">${x.energy||'No energy rating'} · ${sleepText(x.sleep)} sleep</div></div><div class="right"><b>${x.weight??'—'} kg</b><div class="muted">${Math.round(totals(x).kcal)} kcal</div></div></div>`).join('')||'<div class="empty">No logs yet</div>'}</div>`;
